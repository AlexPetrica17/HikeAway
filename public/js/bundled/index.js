...
var _becomeGuide = require("./becomeGuide");
...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    (0, _manageTours.handleExtraLocations)(existing);
});
// DOM ELEMENTS
...
const deleteReviewButtons = document.querySelectorAll('.delete-review-user-btn');
...
if (userPasswordForm) userPasswordForm.addEventListener('submit', async (e)=>{
    ...
    document.querySelector('.btn--save-password').textContent = "Salveaz\u0103 parola";
...
if (tourForm) tourForm.addEventListener('submit', async (e)=>{
    e.preventDefault();
    const mode = tourForm.dataset.mode;
    const id = tourForm.dataset.id;
    try {
        const formData = new FormData();
        // Functie pentru extragerea valorilor
        const getValue = (id)=>{
            const el = document.getElementById(id);
            if (!el) throw new Error(`Elementul cu id="${id}" nu exist\u{103}.`);
            return el.value;
        };
        ...
        formData.append('startLocation.description', getValue('startLocationDescription'));
        formData.append('startDates', getValue('startDate'));
        // Adaugă startLocation
        formData.append('startLocation.address', getValue('startLocationAddress'));
        formData.append('startLocation.coordinates[1]', getValue('startLocationLat'));
        formData.append('startLocation.coordinates[0]', getValue('startLocationLng'));
        const imageCover = document.getElementById('imageCover')?.files[0];
        const images = document.getElementById('images')?.files;
        if (imageCover) formData.append('imageCover', imageCover);
        // Adaugă locații suplimentare
        ...
        for(let i = 0; i < locationCount; i++){
            ...
            // Validare locații
            if (!locationLat || !locationLng || !locationAddress) return (0, _alerts.showAlert)('error', `Loca\u{21B}ia ${i + 1} este incomplet\u{103}.`);
            ...
            formData.append(`locations[${i}][description]`, getValue(`locationDescription-${i}`));
            formData.append(`locations[${i}][day]`, getValue(`locationDay-${i}`));
        }
        // Validare imagini
        if (images?.length > 3) return (0, _alerts.showAlert)('error', "Po\u021Bi \xeenc\u0103rca maxim 3 imagini suplimentare.");
        if (images?.length) [
            ...images
        ].forEach((file)=>formData.append('images', file));
        // Ghid principal
        const leadGuideSelect = document.getElementById('leadGuide');
        if (!leadGuideSelect || !leadGuideSelect.value) return (0, _alerts.showAlert)('error', "Selecteaz\u0103 un ghid principal.");
        const leadGuide = leadGuideSelect.value;
        // Ghiduri secundare
        const guidesSelect = document.getElementById('guides');
        const selectedGuides = [
            ...guidesSelect.options
        ].filter((opt)=>opt.selected).map((opt)=>opt.value);
        // Verificare
        if (!selectedGuides.length) return (0, _alerts.showAlert)('error', "Selecteaz\u0103 cel pu\u021Bin un ghid secundar.");
        // 🔥 INSEREAZĂ leadGuide PRIMUL în guides
        // Include leadGuide în față, fără dubluri
        const allGuides = [
            leadGuide,
        ...
        if (mode === 'edit') await (0, _manageTours.updateTour)(id, formData);
        else await (0, _manageTours.createTour)(formData);
    } catch (err) {
        (0, _alerts.showAlert)('error', err.message || 'Eroare la trimiterea formularului.');
    }
});
if (deleteButtons.length) deleteButtons.forEach((btn)=>btn.addEventListener('click', async ()=>{
        const confirmed = window.confirm("E\u0219ti sigur c\u0103 vrei s\u0103 \u0219tergi acest tur?");
        if (!confirmed) return;
        await (0, _manageTours.deleteTour)(btn.dataset.id);
    }));
if (document.querySelector('.delete-review-btn')) (0, _manageReviews.initReviewDeleteModal)();
...
deleteReviewButtons.forEach((btn)=>{
    btn.addEventListener('click', ()=>{
        ...
        const confirmDelete = confirm("E\u0219ti sigur c\u0103 vrei s\u0103 \u0219tergi aceast\u0103 recenzie?");
        if (confirmDelete) (0, _reviews.deleteReview)(reviewId);
    });
});
deleteBookingButtons.forEach((btn)=>{
    btn.addEventListener('click', ()=>{
        const bookingId = btn.dataset.id;
        const confirmDelete = confirm("E\u0219ti sigur c\u0103 vrei s\u0103 \u0219tergi aceast\u0103 rezervare?");
        if (confirmDelete) (0, _manageBookings.deleteBooking)(bookingId);
    });
});
...
if (contactForm) contactForm.addEventListener('submit', (e)=>{
    ...
    (0, _contactForm.sendContactMessage)(name, email, message);
    contactForm.reset();
});
if (guideForm) guideForm.addEventListener('submit', (e)=>{
    ...
    (0, _becomeGuide.sendGuideApplication)(formData);
    guideForm.reset();
});
userSaveButtons.forEach((btn)=>{
    btn.addEventListener('click', async ()=>{
        ...
        await (0, _updateUser.updateUserRole)(userId, newRole);
    });
});

},{"@babel/polyfill":"dTCHC","./mapbox":"3zDlz","./login":"7yHem","./updateSettings":"l3cGY","./stripe":"10tSC","./register":"eBq2z","./manageTours":"jNQEQ","./alerts":"6Mcnf","./manageReviews":"bgTPK","./reviewForm":"bkCn0","./reviews":"d0spG","./manageBookings":"ojANN","./forgotPassword":"4l6TW","./resetPassword":"eRWSh","./contactForm":"2z6Es","./becomeGuide":"880ko","./updateUser":"575AG"}],"dTCHC":[function(require,module,exports,__globalThis) {
"use strict";
...
const displayMap = (locations)=>{
    mapboxgl.accessToken = 'pk.eyJ1IjoiYWxleHBldHJpY2ExNyIsImEiOiJjbTl1OGs3dnQwNzFkMmlzZzM1N2s5MnE4In0.J6I4VkVHLnNKjiHnq4rWnw';
    ...
    locations.forEach((loc)=>{
        ...
        }).setLngLat(loc.coordinates).setHTML(`<p>Ziua ${loc.day}: ${loc.description}</p>`).addTo(map);
...
const updateSettings = async (data, type)=>{
    try {
        ...
        if (res.data.status === 'success') (0, _alerts.showAlert)('success', 'Profilul a fost actualizat cu succes!');
    } catch (err) {
        (0, _alerts.showAlert)('error', err.response.data.message);
    }
};
...
const bookTour = async (tourId)=>{
    const stripe = Stripe('pk_test_51RJXI0IkXfWtRESAmy2XZgWlwB8QduDzih8t2ahcMZtn5IBhX4irAt8l8fmleLR9pfzz8n8N91YL2pKNcNMumFos00mQ3DFan4');
    try {
...
},{"axios":"jo6P5","./alerts":"6Mcnf","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"eBq2z":[function(require,module,exports,__globalThis) {
/*eslint-disable*/ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
const register = async (name, email, password, passwordConfirm)=>{
    try {
        ...
        if (res.data.status === 'success') {
            (0, _alerts.showAlert)('success', 'Contul a fost creat cu succes!');
            window.setTimeout(()=>{
                location.assign('/');
            }, 1500); //redirectionare la homepage dupa 1,5 secunde de la crearea contului
        }
    } catch (err) {
...
},{"axios":"jo6P5","./alerts":"6Mcnf","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"jNQEQ":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
parcelHelpers.export(exports, "handleExtraLocations", ()=>handleExtraLocations);
var _axios = require("axios");
var _axiosDefault = parcelHelpers.interopDefault(_axios);
var _alerts = require("./alerts");
...
const createTour = async (formData)=>{
    try {
        await (0, _axiosDefault.default).post(`${BASE_URL}`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data'
            }
        });
        (0, _alerts.showAlert)('success', 'Turul a fost creat cu succes!');
        setTimeout(()=>location.assign('/manage-tours'), 1500);
    } catch (err) {
        console.error("\u274C Eroare la creare tur:", err.response?.data || err);
        (0, _alerts.showAlert)('error', err.response?.data?.message || 'Eroare la creare.');
    }
};
const updateTour = async (id, formData)=>{
    try {
        await (0, _axiosDefault.default).patch(`/api/v1/tours/${id}`, formData);
        (0, _alerts.showAlert)('success', "Modific\u0103rile au fost salvate!");
        setTimeout(()=>location.assign('/manage-tours'), 1500);
    } catch (err) {
        (0, _alerts.showAlert)('error', err.response?.data?.message || 'Eroare la actualizare.');
    }
};
const deleteTour = async (id)=>{
    try {
        await (0, _axiosDefault.default).delete(`/api/v1/tours/${id}`);
        (0, _alerts.showAlert)('success', "Turul a fost \u0219ters.");
        setTimeout(()=>location.reload(), 1500);
    } catch (err) {
        (0, _alerts.showAlert)('error', err.response?.data?.message || "Eroare la \u0219tergere.");
    }
};
let renderExtraLocationFields = ()=>{};
const handleExtraLocations = (existingLocations = [])=>{
    ...
    // Dacă avem locații existente (editare), setăm valoarea implicită
    if (existingLocations.length) locationCountInput.value = existingLocations.length;
    const renderExtraLocationFields = ()=>{
        ...
        for(let i = 0; i < count; i++){
            ...
            group.innerHTML = `
        <label class="form__label" for="locationAddress-${i}">Adresa loca\u{21B}iei ${i + 1}</label>
        ...
        <label class="form__label" for="locationDescription-${i}">Descriere loca\u{21B}ie</label>
        <input id="locationDescription-${i}" class="form__input" type="text" name="locations[${i}].description" value="${loc.description || ''}" required>

        <label class="form__label" for="locationDay-${i}">Ziua (numeric\u{103})</label>
...
},{"axios":"jo6P5","./alerts":"6Mcnf","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"bgTPK":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "initReviewDeleteModal", ()=>initReviewDeleteModal);
var _axios = require("axios");
var _axiosDefault = parcelHelpers.interopDefault(_axios);
var _alerts = require("./alerts");
const initReviewDeleteModal = ()=>{
    ...
    const deleteReview = async (id, reason)=>{
        try {
            await (0, _axiosDefault.default).delete(`/api/v1/reviews/${id}`, {
                data: {
                    reason
                }
            });
            (0, _alerts.showAlert)('success', "Recenzia a fost \u0219tears\u0103.");
            setTimeout(()=>location.reload(), 1500);
        } catch (err) {
            (0, _alerts.showAlert)('error', err.response?.data?.message || "Eroare la \u0219tergere.");
        }
    };
    document.querySelectorAll('.delete-review-btn').forEach((btn)=>btn.addEventListener('click', ()=>{
    ...
    confirmBtn?.addEventListener('click', ()=>{
        ...
        if (!reason) return (0, _alerts.showAlert)('error', "Scrie un motiv pentru \u0219tergere!");
...
},{"axios":"jo6P5","./alerts":"6Mcnf","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"bkCn0":[function(require,module,exports,__globalThis) {
/* eslint-disable */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
var _axiosDefault = parcelHelpers.interopDefault(_axios);
var _alerts = require("./alerts");
const addReview = async (data, type)=>{
    try {
        ...
        if (res.data.status === 'success') {
            (0, _alerts.showAlert)('success', 'Recenzia a fost adaugata cu succes!');
            window.setTimeout(()=>{
...
},{"axios":"jo6P5","./alerts":"6Mcnf","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"d0spG":[function(require,module,exports,__globalThis) {
/* eslint-disable */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"axios":"jo6P5","./alerts":"6Mcnf","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"ojANN":[function(require,module,exports,__globalThis) {
/* eslint-disable */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
const deleteBooking = async (bookingId)=>{
    try {
        const res = await (0, _axiosDefault.default).delete(`/api/v1/bookings/${bookingId}`, {
            withCredentials: true
        });
        if (res.status === 204) {
            (0, _alerts.showAlert)('success', "Recenzia a fost \u0219tears\u0103!");
...
const forgotPassword = async (email)=>{
    try {
        ...
        if (res.data.status === 'success') {
            (0, _alerts.showAlert)('success', 'Link-ul a fost trimis cu succes!');
            window.setTimeout(()=>{
                location.assign('/');
            }, 1500);
        }
...
const resetPassword = async (token, password, passwordConfirm)=>{
    try {
        ...
        if (res.data.status === 'success') {
            (0, _alerts.showAlert)('success', "Parola a fost resetat\u0103 cu succes!");
            window.setTimeout(()=>{
                location.assign('/');
            }, 1500);
        }
...
},{"axios":"jo6P5","./alerts":"6Mcnf","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"2z6Es":[function(require,module,exports,__globalThis) {
/* eslint-disable */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "sendContactMessage", ()=>sendContactMessage);
var _axios = require("axios");
var _axiosDefault = parcelHelpers.interopDefault(_axios);
var _alerts = require("./alerts");
const sendContactMessage = async (name, email, message)=>{
    try {
        const res = await (0, _axiosDefault.default).post('/api/v1/contact', {
            name,
            email,
            message
        });
        if (res.data.status === 'success') (0, _alerts.showAlert)('success', 'Mesajul a fost trimis cu succes!');
    } catch (err) {
        (0, _alerts.showAlert)('error', err.response.data.message);
    }
};

},{"axios":"jo6P5","./alerts":"6Mcnf","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"880ko":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "sendGuideApplication", ()=>sendGuideApplication);
var _axios = require("axios");
var _axiosDefault = parcelHelpers.interopDefault(_axios);
var _alerts = require("./alerts");
const sendGuideApplication = async (formData)=>{
    try {
        const res = await (0, _axiosDefault.default).post('/api/v1/guide/application', formData, {
            headers: {
                'Content-Type': 'multipart/form-data'
            }
        });
        if (res.data.status === 'success') (0, _alerts.showAlert)('success', res.data.message);
    } catch (err) {
        (0, _alerts.showAlert)('error', err.response?.data?.message || 'Eroare la trimiterea cererii.');
    }
};

},{"axios":"jo6P5","./alerts":"6Mcnf","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"575AG":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
const updateUserRole = async (userId, newRole)=>{
    try {
        const res = await (0, _axiosDefault.default).patch(`/api/v1/users/${userId}`, {
            role: newRole
        });
        if (res.data.status === 'success') {
            (0, _alerts.showAlert)('success', 'Rolul a fost actualizat cu succes!');
            window.setTimeout(()=>{
                location.reload();
            }, 1500);
        }
    } catch (err) {
        (0, _alerts.showAlert)('error', err.response?.data?.message || 'Eroare la actualizarea rolului.');
    }
};

},{"axios":"jo6P5","./alerts":"6Mcnf","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["f2QDv"], "f2QDv", "parcelRequire62ab", {})
